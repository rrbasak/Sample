...
function Auction() {
  ...
  function setDataForUProduct() {
    const currentProduct = products.filter(
      (product) =>
        (product.is_active === 1 && product.is_paused === 1) ||
        (product.is_active === 0 &&
          product.is_closed === 0 &&
          product.is_paused === 0)
    );
    ...
    <div style={{ marginTop: "20px" }}>
      ...
      <div className="auction-container live">
        {cProduct.map((itr) => (
          <div
            key={itr.id}
            className="card text-bg-dark box"
            style={{
              width: "18rem",
              boxShadow: "0 0 10px 5px rgb(74, 172, 252)",
            }}
          >
            <img
              src={`http://localhost:8080/auction/image/${itr.id}`}
        ...
        {uProduct.map((itr) => (
          ...
          >
            ...
            <div className="card-body">
              ...
              <p>
                <strong>
                  Starting Bid: {new Date(itr.auction_start).toString()}
                </strong>
              </p>
              <Link
